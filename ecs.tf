...
resource "aws_ecs_task_definition" "own_task_definition" {
  family                = "justinlim-terraform-taskdef-tf" # Update accordingly
  ...
  container_definitions = templatefile("./files/task-definition.json", {
    ...
    log_group_name   = "/ecs/justinlim-terraform-taskdef-tf" # Update accordingly
...
resource "aws_ecs_cluster" "own_cluster" {
  name = "justinlim-terraform-cluster-tf" # Update accordingly
}

# Creates an ecs service

resource "aws_ecs_service" "own_service" {
  name             = "justinlim-terraform-svc-tf" # Update accordingly
